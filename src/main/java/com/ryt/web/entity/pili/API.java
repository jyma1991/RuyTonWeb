...
public class API {
	...
	// Create a new stream
    public static StStream createStream(Auth auth, String hubName, String title, String publishKey, String publishSecurity) throws PiliException {
        ...
        if (response.isSuccessful()) {
            ...
            try {
                ...
                return new StStream(jsonObj, auth);
            } catch (IOException e) {
                e.printStackTrace();
                throw new PiliException(e);
            }
    ...
    // Get an exist stream
    public static StStream getStream(Auth auth, String streamId) throws PiliException {
        if (streamId == null) {
    ...
    public static StreamList listStreams(Auth auth, String hubName, String startMarker, long limitCount) throws PiliException {
        ...
        try {
            ...
            String macToken = auth.signRequest(url, "GET", null, null);
            System.out.println("macToken:"+macToken);
            Request request = new Request.Builder()
            .url(url)
            .get()
            .header("User-Agent", Utils.getUserAgent())
    ...
    // List stream
    public static StreamList listStreams(Auth auth, String hubName, String status, String startMarker, long limitCount, String titlePrefix) throws PiliException {
        try {
            hubName = URLEncoder.encode(hubName, Config.UTF8);
    ...
    public static StStream updateStream(Auth auth, String streamId, String publishKey, String publishSecurity, boolean disabled) throws PiliException {
        if (streamId == null) {
            throw new PiliException(MessageConfig.NULL_STREAM_ID_EXCEPTION_MSG);
        }
...
//                System.out.println(jsonObj.toString());
    ...
    // Get recording segments from an exist stream,new method
    ...
    // Get recording segments from an exist stream,old method
    ...
    public static Map<String, String> rtmpLiveUrl(String rtmpPlayHost, String streamId, String[] profiles) {
        ...
        dictionary.put(StStream.ORIGIN, url);
        if (profiles != null) {
    ...
    public static Map<String, String> hlsLiveUrl(String hlsPlayHost, String streamId, String[] profiles) {
        ...
        Map<String, String> dictionary = new HashMap<String, String>();
        dictionary.put(StStream.ORIGIN, url + ".m3u8");
        if (profiles != null) {
            for (String p : profiles) {
    ...
    public static Map<String, String> hlsPlaybackUrl(String hlsPlayHost, String streamId, long startTime, long endTime, String[] profiles) 
        ...
        Map<String, String> dictionary = new HashMap<String, String>();
        dictionary.put(StStream.ORIGIN, url + ".m3u8" + queryPara);
        if (profiles != null) {
            for (String p : profiles) {
    ...
    //**************************************************************new change begin*******************
    //Generate a RTMP publish URL
    public static String publishUrl(StStream stream, long nonce) 
            throws PiliException {
        final String defaultScheme = "rtmp";
    ...
    //Generate RTMP live play URL
    public static Map<String, String> rtmpLiveUrl(StStream stream) {
        final String defaultScheme = "rtmp";
        final String url = String.format("%s://%s/%s/%s",defaultScheme, stream.getPlayRtmp(), stream.getHub(), stream.getTitle());
        Map<String, String> dictionary = new HashMap<String, String>();
        dictionary.put(StStream.ORIGIN, url);
    ...
    //Generate HLS live play URL
    public static Map<String, String> hlsLiveUrl(StStream stream) {
        final String defaultScheme = "http";
        final String url = String.format("%s://%s/%s/%s",defaultScheme, stream.getPlayHls(), stream.getHub(), stream.getTitle());
        Map<String, String> dictionary = new HashMap<String, String>();
        dictionary.put(StStream.ORIGIN, url + ".m3u8");
        String[] profiles = stream.getProfiles();
        if (profiles != null) {
    ...
    /*
    //Generate HLS playback URL
    public static Map<String, String> hlsPlaybackUrl(StStream stream, long startTime, long endTime) 
            throws PiliException {
        final String defaultScheme = "http";
    ...
    public static Map<String, String> httpFlvLiveUrl(StStream stream) {
        ...
        Map<String, String> dictionary = new HashMap<String, String>();
        dictionary.put(StStream.ORIGIN, url + ".flv");
        String[] profiles = stream.getProfiles();
        if (profiles != null) {
            for (String p : profiles) {
	...
	*/
    private static String generateStaticUrl(StStream stream, String scheme) {
        return String.format("%s://%s/%s/%s?key=%s", scheme, stream.getPublishRtmp(), stream.getHub(), stream.getTitle(), stream.getPublishKey());
    }

    private static String generateDynamicUrl(StStream stream, long nonce, String scheme) throws PiliException {
        if (nonce <= 0) {
            nonce = System.currentTimeMillis() / 1000; // the unit should be second
        }
        final String baseUri = "/" + stream.getHub() + "/" + stream.getTitle() + "?nonce=" + nonce;
        String publishToken = null;
        try {
            publishToken = Auth.sign(stream.getPublishKey(), baseUri);
        } catch (SignatureException e) {
            e.printStackTrace();
            throw new PiliException(e);
        }
        return String.format("%s://%s%s&token=%s", scheme, stream.getPublishRtmp(), baseUri, publishToken);
    }
  //**************************************************************new change end*******************
}
