$.extend($.fn.validatebox.defaults.rules, {
	...
	passStrong : {
		validator : function(value) {
			return /^(?![a-zA-z]+$)(?!\d+$)(?![!@#$%^&*]+$)(?![a-zA-z\d]+$)(?![a-zA-z!@#$%^&*]+$)(?![\d!@#$%^&*]+$)[a-zA-Z\d!@#$%^&*]+$/i.test(value);
		},
		message : '密码需由字母+数字+特殊字符组成'
	},
	passMedium : {
		validator : function(value) {
			return /^(?![a-zA-z]+$)(?!\d+$)(?![!@#$%^&*]+$)[a-zA-Z\d!@#$%^&*]+$/i.test(value);
		},
		message : '密码需由字母+数字，字母+特殊字符，数字+特殊字符组成'
	},
	passSmall : {
		validator : function(value) {
			return /^(?:\d+|[a-zA-Z]+|[!@#$%^&*]+)$/i.test(value);
		},
		message : '密码需由纯数字，纯字母，纯特殊字符组成'
	},
	...
	floatOrInt : {// 验证是否为小数或整数
		validator : function(value) {
			return /^(\d{1,5}(,\d\d\d)*(\.\d{1,3}(,\d\d\d)*)?|\d+(\.\d+))?$/i.test(value);
		},
