...
FileProgress.prototype.setComplete = function(up, res) {
    var td = this.fileProgressWrapper.find('td:eq(2)'),
        tdProgress = td.find('.progress');
    var url ="";
    str="<div><strong>上传成功！</strong></div>";
    if (res.url) {
        url = res.url;
    ...
    var showImg = $('<img src="..\\assets\\qnupload\\loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', '..\\assets\\qnupload\\default.png');
