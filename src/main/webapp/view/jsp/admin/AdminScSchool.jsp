            ...
            <form id="searchForm">
                ...
                <table>
                    <tr>
                        ...
                        <td>校园名称</td>
                        <td>
                            <input id="schoolIdSch" name="schoolIdSch" class="textbox" style="width: 120px; height: 22px;" />
                        </td>
                        <td>创建时间</td>
                        <td>
                        ...
                        <td><a id="searchFilter" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-zoom',plain:true">过滤</a> <a href="javascript:void(0);" id="reset_searchFilter" class="easyui-linkbutton" data-options="iconCls:'ext-icon-zoom_out',plain:true">重置过滤</a></td>
                    </tr>
                </table>
            </form>
        </div>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style="background-color:#EAEAEA">
                                    <input id="id" readonly="readonly" value="0" name="id" class="textbox" style="background-color:#EAEAEA;height: 22px;width:40px" />
                                </td>
                                <th></th>
                                <td style="background-color:#EAEAEA">
                            ...
                            <tr>
                                <th>昵称</th>
                                <td>
                                    <input id="nickName" name="user.nickName" class="textbox easyui-validatebox" required="true" style="height: 22px;" />
                                </td>
                                <th>微信</th>
                                <td>
                            ...
                            <tr>
                                <th>性别</th>
                                <td>
                                    <input id="sex" name="user.sex" value='1' class="textbox easyui-validatebox" editable="false" style="height: 22px;width:40px" />
                                </td>
                                <th>序号</th>
                                <td>
                                    <input id="sortId" name="sortId" class="textbox easyui-numberspinner" style="height: 22px;width:40px" />
                                </td>
                            </tr>
                            <tr>
                                <th>地址</th>
                                <td colspan="3"><span style="display: inline-block;width: 30px; text-align:center;">省</span>
                                    <input id="provinceId" name="user.provinceId" class="textbox " editable="false" style="height: 22px;width:25%;margin-left:5px;" />
                                    <span style="display: inline-block;width: 30px; text-align:center;">市</span>
                                    <input id="cityId" name="user.cityId" class="textbox" editable="false" style="height: 22px;width:25%;margin-left:5px" />
                                    <span style="display: inline-block;width: 30px; text-align:center;">区</span>
                                    <input id="districtId" name="user.districtId" class="textbox" editable="false" style="height: 22px;width:25%;margin-left:5px" />
                                </td>
                            </tr>
                            <tr>
                                <th>详细地址</th>
                            ...
                            <tr>
                                <th>校园名称</th>
                                <td>
                                    <input id="schoolName" name="schoolName" class="textbox easyui-validatebox" required="true" style="height: 22px;" />
                                </td>
                                <th>营业执照</th>
                                <td>
                                    <input id="businessLicence" name="businessLicence" class="textbox easyui-validatebox" required="true" style="height: 22px;" />
                                </td>
                            </tr>
                            <tr>
                                <th>学校类型</th>
                                <td>
                                    <input id="schoolTypeId" name="schoolTypeId" class="textbox " editable="false" style="height: 22px;" />
                                </td>
                                <th>传真</th>
                                <td>
        ...
        $(function() {
            ...
            grid = $('#grid').datagrid({
                ...
                onClickRow: function(index, row) {
                    ...
                    //清除省下拉框的值
                    $.getJSON(ctx + 'listAreaProvince.do', {
                        idSch: row.user.provinceId
                    }, function(provinceJson) {
                        if (provinceJson.rows.length == 0) {
                            PS.combobox("clear");
                        }
                    });
                    //根据cityID获取当前用户的city
                    $.getJSON(ctx + 'listAllAreaCity.do', {
                        idSch: row.user.cityId
                    }, function(cityJson) {
                        if (cityJson.rows.length > 0) {
                            CS = $('#cityId').combobox({
                                data: [{
                                    id: cityJson.rows[0].id,
                                    cityName: cityJson.rows[0].cityName,
                                    selected: true
                                }],
                                valueField: 'id',
                                textField: 'cityName',
                    ...
                    $.getJSON(ctx + 'listAllAreaDistrict.do', {
                        idSch: row.user.districtId
                    }, function(districJson) {
                        if (districJson.rows.length > 0) {
                            DS = $('#districtId').combobox({
                                data: [{
                                    id: districJson.rows[0].id,
                                    districtName: districJson.rows[0].districtName,
                                    selected: true
                                }],
                                valueField: 'id',
            ...
            $.getJSON(ctx + 'listAllUser.do', {
                ...
                $('#agentIdSch').combobox({
                    ...
                    onSelect: function(row) {
                        ...
                        //
                        $.getJSON(ctx + 'listAllScSchool.do', {
                            agentIdSch: agId
                        }, function(agJson) {
                            $('#schoolIdSch').combobox({
                                data: agJson.rows,
                                valueField: 'schoolId',
            ...
            //
            $.getJSON(ctx + 'listAllScSchool.do', function(agJson) {
                $('#schoolIdSch').combobox({
                    data: agJson.rows,
                    valueField: 'schoolId',
                    textField: 'schoolName',
            ...
            /*  $.getJSON(ctx+'listAllUser.do',{rolePropertySch:4},function(agJson) {
		             $('#schoolIdSch').combobox({
		            	 data:agJson.rows,
		                 valueField: 'id',
		                 textField: 'trueName',
            ...
            $("#operateclass").click(function() {
                if ($("#schoolId").val().trim().length > 0) {
                    window.location.href = "AdminScSchoolClass.jsp?schoolId=" + $("#schoolId").val();
                } else {
                    $.messager.show({
                        title: message.title.normal,
            ...
            //重置过滤
            $("#reset_searchFilter").click(function() {
                agId = '';
                count = true;
